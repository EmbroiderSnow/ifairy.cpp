...
static inline uint32_t load_packed_bf16_pair(const void * src) {
    uint32_t bits;
    memcpy(&bits, src, sizeof(bits));
    return bits;
}

static inline void store_packed_bf16_pair(void * dst, ggml_bf16_t real, ggml_bf16_t imag) {
    ...
    for (int64_t i03 = 0; i03 < ne03; i03++) {
        for (int64_t i02 = 0; i02 < ne02; i02++) {
            for (int64_t i01 = ith; i01 < ne01; i01 += nth) {
                ...
                ggml_float sum = 0.0;
                for (int64_t i00 = 0; i00 < ne00; i00++) {
                    const uint32_t bits = load_packed_bf16_pair(&x[i00]);
                ...
                for (int64_t i00 = 0; i00 < ne00; i00++) {
                    ...
                    store_packed_bf16_pair(&y[i00], GGML_FP32_TO_BF16(xr), GGML_FP32_TO_BF16(xi));
                }
            }
        }
    }
        ...
        for (int64_t i2 = 0; i2 < ne2; i2++) {  // seq-len
            ...
            for (int64_t i1 = 0; i1 < ne1; i1++) {  // attn-heads
                ...
                for (int64_t i0 = 0; i0 < n_dims; i0++) {
                    ...
                    const uint32_t bits = load_packed_bf16_pair(src);
        ...
        for (int64_t i2 = 0; i2 < ne2; i2++) {      // seq-len
            for (int64_t i1 = 0; i1 < ne1; i1++) {  // attn-heads
                ...
                for (int64_t i0 = 0; i0 < n_dims / 2; i0++) {
                    ...
                    store_packed_bf16_pair(dst_data, x0, x1);
                }
            }
        }
    }
